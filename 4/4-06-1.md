...
> • After the official release of a new suite application, 
you can see the **Settings** button on the far right of the application list after adding an application. Click to modify the following information:   
...
5. * *Restore**: Restore the filter to the default value   
...
> Notes:
> (1) After the application is launched, **Process Name** will be bound to **API Path**. If you need to modify it, you can only change the translation   
...
• * *Hide**: Hide this block and the fields within it 
...
**Default Value** tab: Please separate the selected default values ​​with commas, for example: <User ID>, <User ID>, <User ID>   
...
| Input variables | Variable name | Description | 
| ---------- | - - ------- | -------------------------- | 
| 1 | a | Point the fixed string to 
column a | | Bit B | b | Point the specific field value to b | 
...
Location 
: [Main Menu] > Application Management > Scheduling Settings 
For 

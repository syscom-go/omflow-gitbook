...
#### Final Release 1.2.5.2 (2026-1-6)

* **System**: Enhanced user security protection

#### Final Release 1.2.5.1 (2025-11-3)

* **Single Sign-On**: Fixed an issue where users with Two-Factor Authentication enabled could not log in from mobile devices
* **General Form**: Fixed an issue where the dropdown menu in Related Operations could sometimes fail to scroll down when creating relations
* **System Settings**: Fixed an issue where enabling Administrator Mode caused the webpage to become unresponsive

#### Release Candidate 1.2.5.0 (2025-10-23)

* **System**: Added single sign-on (SSO) support, enabling integration with Windows AD and custom third-party login systems  
* **System**: Added **SaaS Mode** to the installer, allowing online license application after activation  
* **System**: Users must agree to the **Product License Agreement** before accessing the home page after login  
* **System**: Upgraded Bootstrap to version 5  
* **Main Menu**: Customized **Path** items in the main menu now display access permissions based on **Role** settings  
* **Two-Factor Authentication**: Administrators who have not linked their mobile phones cannot enable two-factor authentication  
* **Service Request**: Dropdown menus no longer automatically scroll to the bottom when adding fields during the design phase  
* **Data Collection**: Fixed an issue where the Collector 1.2.4 installer could not connect properly  
* **Application Management**: Disabled ORM syntax in **Code Components** of process design  
* **Application Management**: Fixed an issue allowing the **Task Note** field to be designed in static forms  
* **Application Management**: Optimized the display of dropdown menu settings: **Option Order**, **Maximum Selections**, and **Minimum Selections**  
* **Application Management**: Added new conditions to **Decision Components**: greater than or equal to, and less than or equal to  
* **Application Management**: Fixed an issue where external and created processes did not support **File Fields**  
* **Application Management**: Fixed an issue where **Data Tools** did not exclude deleted data  
* **Application Management**: Adjusted LOG level to **CRITICAL** when an error occurs while executing a **Code Component**  
* **Approval**: When a step is skipped in the approval process, the statuses **Unapproved** and **Unconsulted** remain blank  
* **Approval**: Fixed an issue where others could still fill out opinion forms via URL access  
* **Approval**: Added a new parameter **BASE_URL** to display process URLs in notifications and emails  
* **General Form**: Adjusted print layout to a **Title:Content** width ratio of 3:7  
* **General Form**: Added support for printing blank forms  
* **General Form**: Renamed the **Edit** button in related forms to **Proceed**  
* **General Form**: Fixed an issue where deselecting an item in a sub-query (with only one record) still caused it to be re-populated after submission  
* **User Management**: Added required-field indicators when creating new **Departments** or **Roles**  
* **User Management**: The login page is now set to **Default** when adding a new user  
* **User Management**: Adjusted the tab order in role management to: Settings > Permissions > Users > Departments  
* **User Management**: Enhanced user password security mechanisms  
* **Multilingual**: Corrected several Japanese interface strings  
* **System Settings**: Fixed an issue where an ERROR LOG was generated when LDAP was not configured  
...
* **Data Tables**: Optimized performance by adding indexes to flow-related data tables
* **My Mission**: Date filtering is now based on "Update Time"
* **My Mission**: Fixed issue where delegated users could not view files in delegated tasks
* **Service Request**: Fixed an issue where parentheses in the "Checked" field value prevented service requests from opening
* **Application Management**: In flow design, the "Variable Filter" component can now access the current flow’s api_path
* **Application Management**: In flow design, the "Variable Filter" component can now retrieve the user ID of the delegated submitter
* **Application Management**: Code validation is now performed when publishing an application
* **Application Management**: Fixed an issue where the "Checked" field in read-only mode did not display the "Other" option value
* **Application Management**: Fixed an issue where in "Subtable > Subquery" with single-select mode, leaving it unselected would result in undefined being filled in
* **Application Management**: Flows can now be copied within the same application
* **Application Management**: Form design can now be synchronized across other manual components in the flow
* **Data Collection**: Fixed an issue where dispatch failures in collector flows would still display a success message in the lower-left corner
* **Data Collection**: Fixed an issue where default collector flows failed to dispatch due to missing references in the package tab
* **Approval**: Tools now support writing "Approval Records"; see the [Code Component Tools] section for details
* **Approval**: API and Tools now support fetching approval records for a specific form; see the [Code Component Tools] section
* **Approval**: Added "File Upload" setting, with download available from the approval records
* **Approval**: Fixed issue where the same user could not be consulted multiple times
* **Approval**: Multiple assignees can now be designated during countersigning
* **Approval**: Department assignment is now supported during the "Joint Review" phase in approval design
* **Approval**: Added CC recipient support when sending emails/notifications in approval design
* **Approval**: Added "Import/Export" functionality in approval design
* **General Forms**: Fixed issue where unchecking a single subquery item would still result in the item being submitted
* **General Forms**: Fixed issue where, after removing read-only status from a "DateTime" field, seconds were not restored in the format
* **General Forms**: Fixed issue where proxies were unable to view files
* **User Management**: Organization chart now supports a "Search" function
* **User Management**：Fixed issue where login options were not saved properly when adding new users
* **User Management**：Fixed issue where newly added departments were not automatically assigned a code if left blank
* **System Settings**: Added "Two-Factor Authentication" — requires a mobile verification code during login
* **Standalone Tools**: Fixed issue with the "Form Export" script failing due to restricted admin privileges
* **Reports**: Fixed issue where exporting reports led to a forced logout shortly afterward
* **Japanese Language**: Fixed improper usage of certain permission-related characters not conforming to standard Japanese kanji usage
...
* **People management**: The administrator account has mobile login disabled by default 

...
### LDAP變更時維持「任務、使用者、部門」的指派關係 - 1.2.2 New

此腳本可協助已運行一段時間的OMFLOW系統變更AD，並使新AD使用者及部門能正確取得原有的表單及任務，其中需因應實際AD環境修改以下腳本參數：

#### 連線資訊

ldapdata：JSON格式，其中包含「新使用者、舊使用者、新部門、舊部門」四種設定，每項設定都有以下參數需調整：

1. ldap\_client\_server：填入LDAP Server的 IP 位址
2. ldap\_client\_server\_port：填入LDAP Server的 Port
3. ldap\_base\_dn：填入LDAP Server的基礎DN，如`"DC=dcname,DC=com"`
4. ldap\_search\_dn：填入LDAP Server的搜尋範圍，如`"OU=ouname"`
5. ldap\_bind\_user：填入LDAP Server的使用者名稱
6. ldap\_bind\_user\_password：填入LDAP Server的使用者密碼
7. search\_filter：填入同步對象的搜尋條件，如`"(&(|(objectclass=user)(objectclass=person))"`
...
> ※ 請注意，此腳本必須在「OMFLOW同步新AD」前執行，且事先需備份資料庫。

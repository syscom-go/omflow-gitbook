...
このプログラムの目的は、omfファイルのベース言語を置き換えることであり、使用する前にパラメーターの最初の4行を修正する必要があります。

例えば、test_flow.omfは繁体字中国語で設計されたアプリケーションであり、既にOMFLOWの内蔵機能によって各言語の翻訳がアップロードされています。英語を用いる利用者がこのアプリケーションを利用する必要がある場合、プログラムで「設計」の言語を繁体字中国語から英語に変更することができます。

> ※ omfファイルには、対応する翻訳が含まれている必要があることに注意してください。このプログラムは、翻訳を置き換えるだけであり、自動翻訳ではありません。
...
組織図上で特定の職務/権限を持つユーザーを整理してcsvファイルに保存します。

システムから組織図ファイルとレポートからユーザー情報をエクスポートし、それらの情報をユーザーの権限/職務に組み合わせます。
...
この節では、Pythonコードを提供し、OMFLOWフォームとその添付ファイルを1つのPDFファイルにエクスポートする方法を説明します。
...
実行中、画面にはプロセスのクローズドレコードの総数が表示され、同時にダウンロードタスクが実行されます。 10件のレコードが処理されるごとに、以下の情報が提供されます：
1. 現在の空き容量
2. 現在の環境で利用可能な容量
3. ダウンロードに必要な推定スペース
4. ダウンロードに必要な推定時間

> ※ 上記の項目3と4で提供される予想は、タスクを完了させるためのリソース消費量に基づくため、完全に正確ではない可能性があります。
...
search_conditions.append({'column':'closed','condition':'=','value':True})
```
2.	'formitm_1' フィールド イコール'SYSCOM' の記録だけを検索したい場合は、以下の内容を追加します。
...
このスクリプトは、OMFLOWシステムでLDAPを変更し、新しいLDAPユーザや部署が元のフォームやタスクを正しく取得できるようにします。
以下のスクリプトのパラメータは、実際のLDAP環境に応じて変更する必要があります：

### 接続情報

ldapdata：JSON形式で、「新しいユーザー、古いユーザー、新しい部門、古い部門 」の4つの設定が含まれており、各設定は、次のパラメータを調整する必要があります：

1. ldap_client_server：LDAP ServerのIPアドレスを記入する
2. ldap_client_server_port：LDAP ServerのPortを記入する
3. ldap_base_dn：LDAPサーバーのベースDNを記入する。例：`"DC=dcname,DC=com"`
4. ldap_search_dn：填入LDAP Serverの検索範囲を記入する。例：`"OU=ouname"`
5. ldap_bind_user：填入LDAP Serverのユーザー名をを記入する
6. ldap_bind_user_password：填入LDAP Serverのユーザーパスワード記入する
7. search_filter：同期オブジェクトの検索条件を記入する。例：`"(&(|(objectclass=user)(objectclass=person))"`

### 同期パラメータ

1. nickname：レポートを生成する際にユーザーの表示名として使用する属性のこと。例：`'cn'`
2. username：古いユーザーと新しいユーザーが同一人物かどうかを比較する属性のこと。例：`'sAMAccountName'`
3. groupname：レポートを作成する際に部門の表示名として使用する属性のこと。例：`'name'`
4. groupno：古い部門と新しい部門が同一部門かどうかを比較する属性のこと。例：`'sAMAccountName'`

> ※ 注意：このスクリプトは「OMFLOWが新しいLDAPと同期」の前に実行する必要があり、事前にデータベースをバックアップする必要があります。
...
try:
    ...
    o_sAMAccountName = []
    log = []
    #NEW ADの純sAMAccountName list取得する
        ...
        try:
            ...
            if user_sid != ad_user_n[search_idx]['objectSid']:
                ...
                try:
                    ...
                    log.append(f'ユーザー：{user.username} はad_sidを変更完了')
                except Exception as e:
                    log.append(f'DBにユーザーが見つかりません：{user_sAMAccountName}，データベースに異動があるかご確認ください'+e.__str__())
            else:
                result = '変更なし更'
                ws.append([adUser[nickname],adUser['objectSid'],adUser[username],'',result,'',ad_user_n[search_idx][nickname],ad_user_n[search_idx]['objectSid'],ad_user_n[search_idx][username]])
                log.append(f'ユーザー：{user_sAMAccountName}，ADを移動しても保持され、DBのad_noは変更されません') 
        except ValueError :
            result = 'ADユーザーの削除'
            ws.append([adUser[nickname],adUser['objectSid'],adUser[username],'',result])
            log.append(f'ユーザー：{user_sAMAccountName}，AD同期後に削除されます')
        ...
        for newUser in blance:
            for sa in ad_user_n:
                if newUser == sa.get('sAMAccountName',''):
                    ...
                    log.append(f'ユーザー：{newUser}，AD同期後に追加されます')
                    break
        ...
        try:
            ...
            if group_guid != ad_group_n[search_idx]['objectGUID']:
                try:
                    ...
                    log.append(f'部門：{group.name} name変更完了')
                except Exception as e:
                    log.append(f'DBに部門が見つかりません：{group_guid}，データベースに異動があるかご確認ください'+e.__str__())
            else:
                result = '変更なし'
                ws2.append([adGroup[groupname],adGroup['objectGUID'],adGroup[groupno],'',result,'',ad_group_n[search_idx][groupname],ad_group_n[search_idx]['objectGUID'],ad_group_n[search_idx][groupno]])
                log.append(f'部門：{group_guid}，ADを移動しても保持され，DBのnameは変更されません')    
        except ValueError :
            result = 'AD部門の削除'
            ws2.append([adGroup[groupname],adGroup['objectGUID'],adGroup[groupno],'',result])
            log.append(f'部門：{group_guid}，AD同期後に削除されます')
    ...
    if len(group_blance)>0:
        for newgroup in group_blance:
            ...
            log.append(f'部門：sAMAccountNameは{newgroup}の部門で，AD同期後に追加されます')
